...
export default function InvoiceStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <InvoiceCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function InvoiceCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
