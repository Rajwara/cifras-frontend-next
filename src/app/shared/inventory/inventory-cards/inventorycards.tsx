...
export default function InventoryStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <InventoryCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function InventoryCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
