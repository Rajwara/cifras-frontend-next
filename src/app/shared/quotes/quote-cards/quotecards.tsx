...
export default function QuoteStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <QuoteCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function QuoteCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
