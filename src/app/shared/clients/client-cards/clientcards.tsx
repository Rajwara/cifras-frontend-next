...
export default function ClientStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <ClientCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function ClientCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
