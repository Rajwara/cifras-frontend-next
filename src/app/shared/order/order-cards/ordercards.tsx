...
export default function OrderStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <OrderCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function OrderCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
