...
const data = [
  {
    ...
    label: 'Purchase Order Registered',
  },
  {
    id: 2,
    count: 380,
  ...
  },
  {
    ...
    count: 118,
    icon: <CargoPallet />,
    label: 'Purchase Order Consolidated',
  },
  {
    id: 4,
...
export default function PurchaseOrderStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <PurchaseOrderCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function PurchaseOrderCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
