...
const data = [
  ...
  },
  {
    ...
    count: 380,
    icon: <ShippingBox />,
    label: 'Vendor Delivered',
  },
  {
    id: 3,
    count: 118,
    icon: <CargoPallet />,
    label: 'Vendor Consolidated',
  },
  {
    id: 4,
...
export default function VendorStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <VendorCards key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function VendorCards({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(
